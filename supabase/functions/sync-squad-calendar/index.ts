...
async function syncCalendar(supabase, serviceAccountEmail, privateKey) {
  ...
  // 4. Upsert into squad_calendar_events
  if (allRows.length > 0) {
    ...
    await supabase.from("squad_calendar_events").delete().gte("dia", windowStart).lte("dia", windowEnd);
    for(let i = 0; i < allRows.length; i += 500){
      const batch = allRows.slice(i, i + 500);
      const { error } = await supabase.from("squad_calendar_events").upsert(batch, {
        onConflict: "event_id"
      });
