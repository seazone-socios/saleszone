...
// MKTP: single squad (id=1) with all empreendimentos
...
const META_ACCOUNT_ID = "act_799783985155825";
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    console.log(`sync-mktp-meta-ads v10: lifetime ${sinceLifetime} to ${until}, month ${sinceMonth} to ${until}`);
    ...
    await supabase.from("mktp_meta_ads").delete().eq("snapshot_date", snapshotDate);
    if (rows.length > 0) {
      for(let i = 0; i < rows.length; i += 200){
        ...
        const { error } = await supabase.from("mktp_meta_ads").insert(batch);
    ...
    console.error("sync-mktp-meta-ads fatal:", msg);
    return new Response(JSON.stringify({
      success: false,
      error: msg
    }), {
