...
// Edge Function: sync-szs-presales
// Populates szs_presales_response from Pipedrive API (deals + activities)
// Triggered via POST /functions/v1/sync-szs-presales
//
// Transbordo logic:
...
Deno.serve(async (req) => {
  try {
    ...
    // 2. Read pre-sellers from config (pipeline 37 = SZS)
    ...
    // 6. Replace all data in szs_presales_response
    if (allRows.length > 0) {
    ...
    console.error("sync-szs-presales error:", err);
    return new Response(
