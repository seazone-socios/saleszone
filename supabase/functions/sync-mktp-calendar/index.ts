...
async function syncCalendar(supabase: any, serviceAccountEmail: string, privateKey: CryptoKey) {
  // 1. Read closer rules
  // MKTP closers — hardcoded emails (no squad_closer_rules for MKTP yet)
  const MKTP_CLOSER_EMAILS = ["nevine.saratt@seazone.com.br", "willian.miranda@seazone.com.br"];
  const rules = MKTP_CLOSER_EMAILS.map((email) => ({
    email,
  ...
  // 4. Upsert into mktp_calendar_events
  if (allRows.length > 0) {
    ...
    await supabase.from("mktp_calendar_events").delete().gte("dia", windowStart).lte("dia", windowEnd);
    for(let i = 0; i < allRows.length; i += 500){
      const batch = allRows.slice(i, i + 500);
      const { error } = await supabase.from("mktp_calendar_events").upsert(batch, {
        onConflict: "event_id"
      });
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    console.log(`sync-mktp-calendar completed in ${elapsed}ms`);
    ...
    console.error("sync-mktp-calendar fatal:", msg);
    return new Response(JSON.stringify({
      success: false,
      error: msg
    }), {
