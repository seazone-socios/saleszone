...
// Edge Function: sync-mktp-presales
// Populates mktp_presales_response from Pipedrive API (deals + activities)
// Triggered via POST /functions/v1/sync-mktp-presales
//
// Transbordo logic:
...
Deno.serve(async (req) => {
  try {
    ...
    // 2. Read pre-sellers from config (pipeline 37 = MKTP)
    ...
    // 6. Replace all data in mktp_presales_response
    if (allRows.length > 0) {
    ...
    console.error("sync-mktp-presales error:", err);
    return new Response(
