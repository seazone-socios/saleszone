# Resumo Heartbeat Marketplace

Lê as mensagens mais recentes do canal **#heartbeats-marketplace** no Slack e gera um resumo executivo dos principais problemas e atualizações reportados pelo time de marketplace.

## Parâmetros

- `$ARGUMENTS` (opcional): período em dias para buscar (default: 7). Ex: `/resumo-heartbeat-mktp 14` busca últimos 14 dias.
...
Canal: **#heartbeats-marketplace** — ID: `C0AN9SUPY5P`

Usar a tool `mcp__claude_ai_Slack__slack_read_channel` com:
- `channel_id`: `C0AN9SUPY5P`
...
```
## Resumo Heartbeat Marketplace — DD/MM a DD/MM/YYYY
...
#### Regras de análise
- **Problemas críticos**: queda de captação, churn de proprietários, problemas operacionais (limpeza, check-in), queda de ocupação, bloqueios de plataforma (Airbnb/Booking), atrasos em onboarding
- **Pontos de atenção**: metas em risco, dependências de outras áreas, processos que podem atrasar, sazonalidade impactando
- **Progressos**: novos imóveis captados, melhorias de ocupação, novos mercados abertos, automações implementadas
...
- Quando houver métricas (ocupação, imóveis, receita, churn), incluir os números
