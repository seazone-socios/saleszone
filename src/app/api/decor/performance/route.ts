...
async function fetchDeals(cutoff: string | null): Promise<DealRow[]> {
  ...
  while (true) {
    ...
    if (error) throw new Error(`Supabase error (decor_deals): ${error.message}`);
...
async function fetchPresalesRows(cutoff: string | null): Promise<PresalesRow[]> {
  ...
  while (true) {
    ...
    query = query.range(offset, offset + PAGE - 1);
    const { data, error } = await query;
    if (error) throw new Error(`Supabase error (decor_presales_response): ${error.message}`);
    if (!data || data.length === 0) break;
...
export async function GET(request: Request) {
  try {
    ...
    for (const d of deals) {
      allDealCanals.add((d.empreendimento || "Sem empreendimento"));
    }
    ...
    // --- SQUAD SUMMARIES --- (Decor has 1 squad, all deals)
    ...
    console.error("Decor Performance error:", error);
