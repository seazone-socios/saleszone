...
export async function GET() {
  try {
    // 1. Buscar Baserow — Decor uses decor_baserow_empreendimentos
    const { data: rows, error: errRows } = await supabase
      .from("decor_baserow_empreendimentos")
    ...
    for (const row of rows || []) {
      ...
      // TODO: Decor campaign prefix may differ from [SI]
      if (cn.startsWith("LP ")) {
        tipo = "lp";
    ...
    console.error("Decor Regras MQL error:", error);
