...
export async function GET() {
  try {
    ...
    // Paginar RPC get_mktp_historico_campanhas
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const historicoData: any[] = [];
    let offset = 0;
    while (true) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data, error } = await (supabase.rpc as any)("get_mktp_historico_campanhas").range(offset, offset + 999);
    ...
    const [funnelRes, latestSnapshotRes] = await Promise.all([
      supabase.rpc("get_mktp_planejamento_counts", { months_back: 0, days_back: -1 }),
      supabase
        .from("mktp_meta_ads")
    ...
    console.error("MKTP Historico campanhas error:", msg);
